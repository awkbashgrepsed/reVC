...
static int LuaGiveMoney(lua_State *L)
{
...
static int LuaSetArmour(lua_State *L)
{
	float value = (float)luaL_checknumber(L, 1);
...
static int LuaGiveWeapon(lua_State *L)
{
	int weapon = (int)luaL_checkinteger(L, 1);
	int ammo = (int)luaL_optinteger(L, 2, 9999);
	if (Player()) Player()->GiveWeapon((eWeaponType)weapon, ammo);
	return 0;
}
...
static std::vector<std::string> FindScripts()
{
	...
	intptr_t handle = _findfirst("scripts\\*.lua", &data);
	if (handle == -1) return result;
	do {
		if (!(data.attrib & _A_SUBDIR)) result.push_back(std::string("scripts\\") + data.name);
	...
	while ((entry = readdir(dir)) != nullptr) {
		...
		if (name.size() > 4 && name.substr(name.size() - 4) == ".lua")
			result.push_back(std::string("scripts/") + name);
	}
	closedir(dir);
#endif
	return result;
...
static void Worker()
{
	// Let the game finish startup before scripts touch game objects.
	...
	std::vector<std::string> scripts = FindScripts();
		...
		while (running.load(std::memory_order_relaxed)) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				if (PSGLOBAL(window) != nil)
					down = glfwGetKey(PSGLOBAL(window), GLFW_KEY_F5 + i) == GLFW_PRESS;
#endif
				if (down && !oldKeys[i]) CallKey(L, names[i]);
				oldKeys[i] = down;
			}
